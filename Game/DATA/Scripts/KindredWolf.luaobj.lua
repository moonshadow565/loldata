...
function OnTargetLost()
  ...
  newTarget = FindTargetInAcRWithFilter(UnitTagFlags.Champion + UnitTagFlags.Minion + UnitTagFlags.Monster, UnitTagFlags.Structure + UnitTagFlags.Ward)
...
function TimerScanDistance()
  ...
  L0_2 = GetState()
  bNoEnemiesNear = FindTargetInAcRWithFilter(UnitTagFlags.Champion + UnitTagFlags.Minion + UnitTagFlags.Monster, UnitTagFlags.Structure + UnitTagFlags.Ward) == nil
  if L0_2 == AI_PET_IDLE and distanceToOwner > GetPetReturnRadius(me) and bNoEnemiesNear then
    SetStateAndCloseToTarget(AI_PET_RETURN, tempOwner)
    return
  end
  if (L0_2 == AI_PET_RETURN or L0_2 == AI_PET_HARDRETURN) and distanceToOwner <= GetPetReturnRadius(me) then
    NetSetState(AI_PET_IDLE)
    return
  end
  if not OwnerIsMoving() and (L0_2 == AI_PET_MOVE or L0_2 == AI_PET_ATTACKMOVE and bNoEnemiesNear) then
    NetSetState(AI_PET_IDLE)
    return
  end
  if not IsMoving() and L0_2 == AI_PET_HARDMOVE then
    NetSetState(AI_PET_HARDIDLE)
    return
  end
  if L0_2 == AI_PET_SPAWNING and IsNetworkLocal() then
    NetSetState(AI_PET_IDLE)
  end
end
function TimerFindEnemies()
  ...
  L0_3 = GetState()
  if L0_3 == AI_PET_MOVE or L0_3 == AI_PET_HARDMOVE or L0_3 == AI_PET_HARDSTOP then
    return
  end
  if L0_3 == AI_PET_IDLE or L0_3 == AI_PET_RETURN or L0_3 == AI_PET_ATTACKMOVE or L0_3 == AI_PET_HARDIDLE or L0_3 == AI_PET_HOLDPOSITION then
    newTarget = FindTargetInAcRWithFilter(UnitTagFlags.Champion + UnitTagFlags.Minion + UnitTagFlags.Monster, UnitTagFlags.Structure + UnitTagFlags.Ward)
    if newTarget == nil then
      TurnOffAutoAttack(STOPREASON_TARGET_LOST)
      return
    end
    if L0_3 ~= AI_PET_HARDATTACK and L0_3 ~= AI_PET_HARDMOVE and L0_3 ~= AI_PET_HARDIDLE and L0_3 ~= AI_PET_HARDIDLE_ATTACKING and L0_3 ~= AI_PET_HARDRETURN then
      AIScriptSpellBuffRemove(me, "PetCommandParticle")
    end
    if L0_3 == AI_PET_IDLE then
      SetStateAndCloseToTarget(AI_PET_ATTACK, newTarget)
    elseif L0_3 == AI_PET_RETURN then
      SetStateAndCloseToTarget(AI_PET_RETURN_ATTACKING, newTarget)
    elseif L0_3 == AI_PET_ATTACKMOVE then
      SetStateAndCloseToTarget(AI_PET_ATTACKMOVE_ATTACKING, newTarget)
    elseif L0_3 == AI_PET_HARDIDLE then
      NetSetState(AI_PET_HARDIDLE_ATTACKING)
      SetTarget(newTarget)
    elseif L0_3 == AI_PET_HOLDPOSITION then
      NetSetState(AI_PET_HOLDPOSITION_ATTACKING)
      SetTarget(newTarget)
    end
  end
  if L0_3 == AI_PET_ATTACK or L0_3 == AI_PET_HARDATTACK or L0_3 == AI_PET_RETURN_ATTACKING or L0_3 == AI_PET_ATTACKMOVE_ATTACKING or L0_3 == AI_PET_HARDIDLE_ATTACKING or L0_3 == AI_PET_HOLDPOSITION_ATTACKING or L0_3 == AI_TAUNTED then
