...
function L0_0(A0_2, A1_3, A2_4)
  ...
  if A0_2 == ORDER_ATTACKTO then
    ...
    SetStateAndCloseToTarget(AI_PET_ATTACK, A2_4)
    return true
  end
  if A0_2 == ORDER_MOVETO then
    if DistanceBetweenObjectAndPoint(me, A1_3) > FAR_MOVMEMENT_DISTANCE then
  ...
  if A0_2 == ORDER_PETHARDATTACK then
    TurnOffAutoAttack(STOPREASON_TARGET_LOST)
    SetStateAndCloseToTarget(AI_PET_HARDATTACK, A2_4)
    return true
  end
  if A0_2 == ORDER_PETHARDMOVE then
    SetStateAndMove(AI_PET_HARDMOVE, A1_3)
    return true
  end
