local L0_0
L0_0 = 180
MAX_MINIONS_EVER = L0_0
L0_0 = 100
TEAM_ORDER = L0_0
L0_0 = 200
TEAM_CHAOS = L0_0
L0_0 = 1
ORDER_HQ = L0_0
L0_0 = 2
CHAOS_HQ = L0_0
L0_0 = 3
FRONT_TOWER = L0_0
L0_0 = 2
MIDDLE_TOWER = L0_0
L0_0 = 1
BACK_TOWER = L0_0
L0_0 = 4
HQ_TOWER2 = L0_0
L0_0 = 5
HQ_TOWER1 = L0_0
L0_0 = 0
RIGHT_LANE = L0_0
L0_0 = 1
CENTER_LANE = L0_0
L0_0 = 2
LEFT_LANE = L0_0
L0_0 = 90
INITIAL_TIME_TO_SPAWN = L0_0
L0_0 = 3
CANNON_MINION_SPAWN_FREQUENCY = L0_0
L0_0 = 2090
INCREASE_CANNON_RATE_TIMER = L0_0
L0_0 = 0
MINION_HEALTH_DENIAL_PERCENT = L0_0
L0_0 = 90
UPGRADE_MINION_TIMER = L0_0
L0_0 = 1250
EXP_GIVEN_RADIUS = L0_0
L0_0 = 1250
GOLD_GIVEN_RADIUS = L0_0
L0_0 = 240
DISABLE_MINION_SPAWN_BASE_TIME = L0_0
L0_0 = 0
DISABLE_MINION_SPAWN_MAG_TIME = L0_0
L0_0 = -1
LAST_WAVE = L0_0
L0_0 = "none"
SPECIAL_MINION_MODE = L0_0
L0_0 = false
HQTurretAttackable = L0_0
L0_0 = {}
L0_0.WaveSpawnRate = 30000
L0_0.SingleMinionSpawnDelay = 800
L0_0.ExpRadius = EXP_GIVEN_RADIUS
L0_0.GoldRadius = GOLD_GIVEN_RADIUS
SpawnTable = L0_0
L0_0 = {}
L0_0.DefaultNumPerWave = 3
L0_0.Armor = 0
L0_0.ArmorUpgrade = 1
L0_0.MagicResistance = 0
L0_0.MagicResistanceUpgrade = 0.625
L0_0.HPBonus = 0
L0_0.HPUpgrade = 10
L0_0.HPInhibitor = 0
L0_0.DamageBonus = 0
L0_0.DamageUpgrade = 0.5
L0_0.DamageInhibitor = 0
L0_0.ExpGiven = 64
L0_0.ExpBonus = 0
L0_0.ExpUpgrade = 0
L0_0.ExpInhibitor = 0
L0_0.GoldGiven = 18.8
L0_0.GoldBonus = 0
L0_0.GoldUpgrade = 0.2
L0_0.GoldInhibitor = 0
L0_0.GoldShared = 0
L0_0.GoldShareUpgrade = 0
L0_0.GoldMaximumBonus = 12
L0_0.LocalGoldGiven = 0
L0_0.LocalGoldBonus = 0
MeleeDefaultMinionInfo = L0_0
L0_0 = {}
L0_0.DefaultNumPerWave = 3
L0_0.Armor = 0
L0_0.ArmorUpgrade = 0.625
L0_0.MagicResistance = 0
L0_0.MagicResistanceUpgrade = 1
L0_0.HPBonus = 0
L0_0.HPUpgrade = 7.5
L0_0.HPInhibitor = 0
L0_0.DamageBonus = 0
L0_0.DamageUpgrade = 1
L0_0.DamageInhibitor = 0
L0_0.ExpGiven = 32
L0_0.ExpBonus = 0
L0_0.ExpUpgrade = 0
L0_0.ExpInhibitor = 0
L0_0.GoldGiven = 13.8
L0_0.GoldBonus = 0
L0_0.GoldUpgrade = 0.2
L0_0.GoldInhibitor = 0
L0_0.GoldShared = 0
L0_0.GoldShareUpgrade = 0
L0_0.GoldMaximumBonus = 8
L0_0.LocalGoldGiven = 0
L0_0.LocalGoldBonus = 0
CasterDefaultMinionInfo = L0_0
L0_0 = {}
L0_0.DefaultNumPerWave = 0
L0_0.Armor = 0
L0_0.ArmorUpgrade = 1.5
L0_0.MagicResistance = 0
L0_0.MagicResistanceUpgrade = 1.5
L0_0.HPBonus = 0
L0_0.HPUpgrade = 13.5
L0_0.HPInhibitor = 0
L0_0.DamageBonus = 0
L0_0.DamageUpgrade = 1.5
L0_0.DamageInhibitor = 0
L0_0.ExpGiven = 100
L0_0.ExpBonus = 0
L0_0.ExpUpgrade = 0
L0_0.ExpInhibitor = 0
L0_0.GoldGiven = 39.5
L0_0.GoldBonus = 0
L0_0.GoldUpgrade = 0.5
L0_0.GoldInhibitor = 0
L0_0.GoldShared = 0
L0_0.GoldShareUpgrade = 0
L0_0.GoldMaximumBonus = 30
L0_0.LocalGoldGiven = 0
L0_0.LocalGoldBonus = 0
CannonDefaultMinionInfo = L0_0
L0_0 = {}
L0_0.DefaultNumPerWave = 0
L0_0.Armor = 0
L0_0.ArmorUpgrade = 0
L0_0.MagicResistance = 0
L0_0.MagicResistanceUpgrade = 0
L0_0.HPBonus = 0
L0_0.HPUpgrade = 100
L0_0.HPInhibitor = 0
L0_0.DamageBonus = 0
L0_0.DamageUpgrade = 5
L0_0.DamageInhibitor = 0
L0_0.ExpGiven = 100
L0_0.ExpBonus = 0
L0_0.ExpUpgrade = 0
L0_0.ExpInhibitor = 0
L0_0.GoldGiven = 39.5
L0_0.GoldBonus = 0
L0_0.GoldUpgrade = 0.5
L0_0.GoldInhibitor = 0
L0_0.GoldShared = 0
L0_0.GoldShareUpgrade = 0
L0_0.GoldMaximumBonus = 30
L0_0.LocalGoldGiven = 0
L0_0.LocalGoldBonus = 0
SuperDefaultMinionInfo = L0_0
L0_0 = {}
L0_0.MinionName = "SRU_OrderMinionMelee"
L0_0.DefaultInfo = MeleeDefaultMinionInfo
OrderMeleeMinionInfo = L0_0
L0_0 = {}
L0_0.MinionName = "SRU_ChaosMinionMelee"
L0_0.DefaultInfo = MeleeDefaultMinionInfo
ChaosMeleeMinionInfo = L0_0
L0_0 = {}
L0_0.MinionName = "SRU_OrderMinionRanged"
L0_0.DefaultInfo = CasterDefaultMinionInfo
OrderCasterMinionInfo = L0_0
L0_0 = {}
L0_0.MinionName = "SRU_ChaosMinionRanged"
L0_0.DefaultInfo = CasterDefaultMinionInfo
ChaosCasterMinionInfo = L0_0
L0_0 = {}
L0_0.MinionName = "SRU_OrderMinionSiege"
L0_0.DefaultInfo = CannonDefaultMinionInfo
OrderCannonMinionInfo = L0_0
L0_0 = {}
L0_0.MinionName = "SRU_ChaosMinionSiege"
L0_0.DefaultInfo = CannonDefaultMinionInfo
ChaosCannonMinionInfo = L0_0
L0_0 = {}
L0_0.MinionName = "SRU_OrderMinionSuper"
L0_0.DefaultInfo = SuperDefaultMinionInfo
OrderSuperMinionInfo = L0_0
L0_0 = {}
L0_0.MinionName = "SRU_ChaosMinionSuper"
L0_0.DefaultInfo = SuperDefaultMinionInfo
ChaosSuperMinionInfo = L0_0
L0_0 = {}
L0_0.IsDestroyed = false
L0_0.NumOfSpawnDisables = 0
L0_0.WillSpawnSuperMinion = 0
L0_0.SpawnOrderMinionNames = {
  "Super",
  "Melee",
  "Cannon",
  "Caster"
}
DefaultBarrackInfo = L0_0
L0_0 = {}
L0_0.Melee = OrderMeleeMinionInfo
L0_0.Caster = OrderCasterMinionInfo
L0_0.Cannon = OrderCannonMinionInfo
L0_0.Super = OrderSuperMinionInfo
DefaultOrderMinionInfoTable = L0_0
L0_0 = {}
L0_0.Melee = ChaosMeleeMinionInfo
L0_0.Caster = ChaosCasterMinionInfo
L0_0.Cannon = ChaosCannonMinionInfo
L0_0.Super = ChaosSuperMinionInfo
DefaultChaosMinionInfoTable = L0_0
L0_0 = {}
L0_0.DefaultInfo = DefaultBarrackInfo
L0_0.DefaultMinionInfoTable = DefaultOrderMinionInfoTable
OrderBarrack0 = L0_0
L0_0 = {}
L0_0.DefaultInfo = DefaultBarrackInfo
L0_0.DefaultMinionInfoTable = DefaultOrderMinionInfoTable
OrderBarrack1 = L0_0
L0_0 = {}
L0_0.DefaultInfo = DefaultBarrackInfo
L0_0.DefaultMinionInfoTable = DefaultOrderMinionInfoTable
OrderBarrack2 = L0_0
L0_0 = {}
L0_0.DefaultInfo = DefaultBarrackInfo
L0_0.DefaultMinionInfoTable = DefaultChaosMinionInfoTable
ChaosBarrack0 = L0_0
L0_0 = {}
L0_0.DefaultInfo = DefaultBarrackInfo
L0_0.DefaultMinionInfoTable = DefaultChaosMinionInfoTable
ChaosBarrack1 = L0_0
L0_0 = {}
L0_0.DefaultInfo = DefaultBarrackInfo
L0_0.DefaultMinionInfoTable = DefaultChaosMinionInfoTable
ChaosBarrack2 = L0_0
function L0_0(A0_1, A1_2, A2_3, A3_4)
  local L4_5, L5_6, L6_7, L7_8, L8_9, L9_10, L10_11
  if A1_2 ~= nil then
    L4_5 = type
    L5_6 = A1_2
    L4_5 = L4_5(L5_6)
    if L4_5 == "table" then
      L4_5, L5_6 = nil, nil
      if A0_1 == nil then
        A0_1 = L6_7
      end
      for L9_10, L10_11 in L6_7(L7_8) do
        if A2_3 == true and type(L10_11) == "table" then
          A0_1[L9_10] = AppendTable(A0_1[L9_10], L10_11, A3_4, A3_4)
        else
          A0_1[L9_10] = L10_11
        end
      end
    end
  end
  return A0_1
end
AppendTable = L0_0
function L0_0(A0_12)
  local L1_13, L2_14, L3_15, L4_16, L5_17, L6_18, L7_19, L8_20, L9_21
  L1_13 = AppendTable
  L2_14 = A0_12
  L1_13 = L1_13(L2_14, L3_15, L4_16, L5_17)
  A0_12 = L1_13
  L1_13 = AppendTable
  L2_14 = A0_12.MinionInfoTable
  L1_13 = L1_13(L2_14, L3_15, L4_16, L5_17)
  A0_12.MinionInfoTable = L1_13
  L1_13 = A0_12.MinionInfoTable
  if L1_13 ~= nil then
    L1_13 = A0_12.MinionInfoTable
    L2_14 = nil
    for L7_19, L8_20 in L4_16(L5_17) do
      L9_21 = L1_13[L7_19]
      if L9_21.DefaultInfo ~= nil then
        AppendTable(L9_21, L9_21.DefaultInfo, true, true)
        L9_21.NumPerWave = L9_21.DefaultNumPerWave
        L9_21.GoldGivenBase = L9_21.GoldGiven
        L9_21.ExpGivenBase = L9_21.ExpGiven
      end
    end
  end
  L1_13 = A0_12.Overrides
  if L1_13 ~= nil then
    L1_13, L2_14 = nil, nil
    for L6_18, L7_19 in L3_15(L4_16) do
      L8_20 = type
      L9_21 = A0_12[L6_18]
      L8_20 = L8_20(L9_21)
      if L8_20 == "table" then
        L8_20 = type
        L9_21 = L7_19
        L8_20 = L8_20(L9_21)
        if L8_20 == "table" then
          L8_20 = AppendTable
          L9_21 = A0_12[L6_18]
          L8_20(L9_21, L7_19, true, true)
        end
      else
        L8_20 = type
        L9_21 = L7_19
        L8_20 = L8_20(L9_21)
        if L8_20 ~= "table" then
          A0_12[L6_18] = L7_19
        end
      end
    end
  end
  return A0_12
end
InitBarrackInfo = L0_0
L0_0 = {
  InitBarrackInfo(OrderBarrack0),
  InitBarrackInfo(OrderBarrack1),
  InitBarrackInfo(OrderBarrack2)
}
OrderBarracksBonuses = L0_0
L0_0 = {
  InitBarrackInfo(ChaosBarrack0),
  InitBarrackInfo(ChaosBarrack1),
  InitBarrackInfo(ChaosBarrack2)
}
ChaosBarracksBonuses = L0_0
L0_0 = {
  "SRU_Dragon",
  "SRU_Krug",
  "SRU_KrugMini",
  "SRU_Red",
  "SRU_RedMini",
  "SRU_Razorbeak",
  "SRU_RazorbeakMini",
  "SRU_Murkwolf",
  "SRU_MurkwolfMini",
  "SRU_Blue",
  "SRU_BlueMini",
  "SRU_Gromp",
  "SRU_Baron"
}
NeutralMinionNames = L0_0
function L0_0()
  local L0_22, L1_23
  L0_22 = {}
  L0_22.Turret3 = true
  L0_22.Turret2 = true
  L0_22.Turret1 = true
  L0_22.Barracks = true
  return L0_22
end
CreateLaneBuildingTable = L0_0
L0_0 = {
  CreateLaneBuildingTable(),
  CreateLaneBuildingTable(),
  (CreateLaneBuildingTable())
}
L0_0.HQTower2 = true
L0_0.HQTower1 = true
L0_0.HQ = true
OrderBuildingStatus = L0_0
L0_0 = {
  CreateLaneBuildingTable(),
  CreateLaneBuildingTable(),
  (CreateLaneBuildingTable())
}
L0_0.HQTower2 = true
L0_0.HQTower1 = true
L0_0.HQ = true
ChaosBuildingStatus = L0_0
L0_0 = 0
TotalNumberOfMinions = L0_0
L0_0 = 3
totalNumberOfChaosBarracks = L0_0
L0_0 = 3
totalNumberOfOrderBarracks = L0_0
function L0_0(A0_24)
  local L1_25, L2_26
  for _FORV_6_, _FORV_7_ in pairs(A0_24.DefaultMinionInfoTable) do
    if _FORV_7_.PreloadedCharacter == nil then
      _FORV_7_.PreloadedCharacter = 1
      PreloadCharacter(_FORV_7_.MinionName)
    end
  end
end
PreloadBarrackCharacters = L0_0
function L0_0()
  local L0_27, L1_28, L2_29, L3_30, L4_31, L5_32, L6_33, L7_34
  for L6_33, L7_34 in L3_30(L4_31) do
    PreloadBarrackCharacters(L7_34)
  end
  for L6_33, L7_34 in L3_30(L4_31) do
    PreloadBarrackCharacters(L7_34)
  end
  L3_30(L4_31)
  L3_30(L4_31)
  L3_30(L4_31)
  L3_30(L4_31)
  L3_30(L4_31)
  L3_30(L4_31)
  L3_30(L4_31)
  L3_30(L4_31)
  L3_30(L4_31)
  L3_30(L4_31)
  L7_34 = L4_31()
  L3_30(L4_31, L5_32, L6_33, L7_34, L4_31())
  L3_30(L4_31, L5_32)
  L3_30()
  L3_30(L4_31, L5_32)
  L3_30.WaveSpawnRate = 30000
  L3_30.SingleMinionSpawnDelay = 800
  L3_30.ExpRadius = L4_31
  L3_30.GoldRadius = L4_31
end
OnLevelInit = L0_0
function L0_0()
  InitTimer("UpgradeMinionTimer", UPGRADE_MINION_TIMER, true)
  InitTimer("IncreaseCannonMinionSpawnRate", INCREASE_CANNON_RATE_TIMER, false)
  InitTimer("AllowDamageOnBuildings", 10, false)
end
OnLevelInitServer = L0_0
function L0_0()
  LoadLevelScriptIntoScript("CreateLevelProps.lua", 11)
  CreateLevelProps()
end
OnPostLevelLoad = L0_0
function L0_0(A0_35)
  if A0_35 == TEAM_CHAOS then
    return TEAM_ORDER
  else
    return TEAM_CHAOS
  end
end
OppositeTeam = L0_0
function L0_0()
  local L0_36, L1_37, L2_38, L3_39, L4_40, L5_41, L6_42, L7_43, L8_44, L9_45, L10_46, L11_47
  for L4_40 = 1, 2 do
    if L4_40 == 1 then
      L0_36 = OrderBarracksBonuses
    else
      L0_36 = ChaosBarracksBonuses
    end
    for L8_44 = 1, 3 do
      L9_45, L10_46 = nil, nil
      L11_47 = L0_36[L8_44]
      L11_47 = L11_47.MinionInfoTable
      for _FORV_15_, _FORV_16_ in pairs(L11_47) do
        _FORV_16_.HPBonus = _FORV_16_.HPBonus + _FORV_16_.HPUpgrade
        _FORV_16_.DamageBonus = _FORV_16_.DamageBonus + _FORV_16_.DamageUpgrade
        _FORV_16_.GoldBonus = _FORV_16_.GoldBonus + _FORV_16_.GoldUpgrade
        if _FORV_16_.GoldBonus > _FORV_16_.GoldMaximumBonus then
          _FORV_16_.GoldBonus = _FORV_16_.GoldMaximumBonus
        end
        _FORV_16_.LocalGoldBonus = _FORV_16_.LocalGoldBonus + _FORV_16_.GoldShareUpgrade
        _FORV_16_.Armor = _FORV_16_.Armor + _FORV_16_.ArmorUpgrade
        _FORV_16_.MagicResistance = _FORV_16_.MagicResistance + _FORV_16_.MagicResistanceUpgrade
        _FORV_16_.ExpBonus = _FORV_16_.ExpBonus + _FORV_16_.ExpUpgrade
        _FORV_16_.GoldGiven = _FORV_16_.GoldBonus + _FORV_16_.GoldGivenBase
        _FORV_16_.LocalGoldGiven = _FORV_16_.LocalGoldBonus + _FORV_16_.GoldShared
        _FORV_16_.ExpGiven = _FORV_16_.ExpBonus + _FORV_16_.ExpGivenBase
      end
    end
  end
end
UpgradeMinionTimer = L0_0
function L0_0()
  local L0_48, L1_49, L2_50, L3_51, L4_52, L5_53, L6_54, L7_55
  for L3_51 = RIGHT_LANE, LEFT_LANE do
    for L7_55 = BACK_TOWER, HQ_TOWER1 do
      orderTurret = GetTurret(TEAM_ORDER, L3_51, L7_55)
      if orderTurret ~= nil then
        if L7_55 == FRONT_TOWER then
          SetInvulnerable(orderTurret, false)
          SetTargetable(orderTurret, true)
        else
          SetInvulnerable(orderTurret, true)
          SetNotTargetableToTeam(orderTurret, true, TEAM_CHAOS)
        end
      end
      chaosTurret = GetTurret(TEAM_CHAOS, L3_51, L7_55)
      if chaosTurret ~= nil then
        if L7_55 == FRONT_TOWER then
          SetInvulnerable(chaosTurret, false)
          SetTargetable(chaosTurret, true)
        else
          SetInvulnerable(chaosTurret, true)
          SetNotTargetableToTeam(chaosTurret, true, TEAM_ORDER)
        end
      end
    end
  end
end
AllowDamageOnBuildings = L0_0
function L0_0(A0_56)
  for _FORV_4_, _FORV_5_ in pairs(A0_56) do
    _FORV_5_.NumPerWave = _FORV_5_.DefaultNumPerWave
  end
end
ResetToDefaultWaveCounts = L0_0
function L0_0(A0_57)
  for _FORV_4_, _FORV_5_ in pairs(A0_57) do
    _FORV_5_.NumPerWave = 0
  end
end
ClearCurrentWaveCounts = L0_0
function L0_0(A0_58, A1_59)
  local L2_60, L3_61, L4_62
  L2_60 = 0
  L3_61 = TEAM_ORDER
  if A1_59 == L3_61 then
    L3_61 = OrderBarracksBonuses
    L4_62 = A0_58 + 1
    L2_60 = L3_61[L4_62]
  else
    L3_61 = ChaosBarracksBonuses
    L4_62 = A0_58 + 1
    L2_60 = L3_61[L4_62]
  end
  L3_61 = {}
  L4_62 = SpawnTable
  L4_62 = L4_62.WaveSpawnRate
  L3_61.WaveSpawnRate = L4_62
  L4_62 = SpawnTable
  L4_62 = L4_62.SingleMinionSpawnDelay
  L3_61.SingleMinionSpawnDelay = L4_62
  L4_62 = L2_60.IsDestroyed
  L3_61.IsDestroyed = L4_62
  L4_62 = L2_60.MinionInfoTable
  L3_61.MinionInfoTable = L4_62
  ReturnTable = L3_61
  L3_61 = ReturnTable
  return L3_61
end
GetInitSpawnInfo = L0_0
function L0_0(A0_63, A1_64, A2_65, A3_66, A4_67)
  local L5_68, L6_69, L7_70
  L5_68 = 0
  L6_69 = TEAM_ORDER
  if A3_66 == L6_69 then
    L6_69 = OrderBarracksBonuses
    L7_70 = A0_63 + 1
    L5_68 = L6_69[L7_70]
  else
    L6_69 = ChaosBarracksBonuses
    L7_70 = A0_63 + 1
    L5_68 = L6_69[L7_70]
  end
  L6_69 = L5_68.MinionInfoTable
  L6_69 = L6_69.Super
  L7_70 = L5_68.MinionInfoTable
  L7_70 = L7_70.Cannon
  ResetToDefaultWaveCounts(L5_68.MinionInfoTable)
  if A1_64 % CANNON_MINION_SPAWN_FREQUENCY == 0 then
    L7_70.NumPerWave = L7_70.NumPerWave + 1
  end
  if A4_67 ~= LAST_WAVE then
    BARRACKSCOUNT = 6
    totalMinionsRemaining = MAX_MINIONS_EVER - GetTotalTeamMinionsSpawned()
    if totalMinionsRemaining <= BARRACKSCOUNT * 7 then
      if 0 >= totalMinionsRemaining then
        SPECIAL_MINION_MODE = "None"
      elseif totalMinionsRemaining >= BARRACKSCOUNT then
        SPECIAL_MINION_MODE = "2MeleeMinions"
      else
        SPECIAL_MINION_MODE = "None"
      end
    else
      SPECIAL_MINION_MODE = ""
    end
    LAST_WAVE = A4_67
  end
  if L5_68.WillSpawnSuperMinion == 1 then
    if A3_66 == TEAM_ORDER and totalNumberOfChaosBarracks == 0 or A3_66 == TEAM_CHAOS and totalNumberOfOrderBarracks == 0 then
      L6_69.NumPerWave = 2
    else
      L6_69.NumPerWave = 1
    end
    L7_70.NumPerWave = 0
  end
  if SPECIAL_MINION_MODE == "2MeleeMinions" then
    ClearCurrentWaveCounts(L5_68.MinionInfoTable)
    L6_69.NumPerWave = L5_68.MinionInfoTable.Super.NumPerWave
    L5_68.MinionInfoTable.Melee.NumPerWave = math.max(2 - L5_68.MinionInfoTable.Super.NumPerWave, 0)
  elseif SPECIAL_MINION_MODE == "None" then
    ClearCurrentWaveCounts(L5_68.MinionInfoTable)
  end
  ReturnTable = {
    NewFormat = true,
    SpawnOrderMinionNames = L5_68.SpawnOrderMinionNames,
    IsDestroyed = L5_68.IsDestroyed,
    ExperienceRadius = SpawnTable.ExpRadius,
    GoldRadius = SpawnTable.GoldRadius,
    MinionInfoTable = L5_68.MinionInfoTable
  }
  return ReturnTable
end
GetMinionSpawnInfo = L0_0
function L0_0(A0_71, A1_72, A2_73)
  local L3_74, L4_75
  L4_75 = TEAM_ORDER
  if A0_71 == L4_75 then
    L3_74 = OrderBuildingStatus
  else
    L3_74 = ChaosBuildingStatus
  end
  L4_75 = FRONT_TOWER
  if A2_73 == L4_75 then
    L4_75 = A1_72 + 1
    L4_75 = L3_74[L4_75]
    L4_75.Turret3 = false
    L4_75 = GetTurret
    L4_75 = L4_75(A0_71, A1_72, MIDDLE_TOWER)
    SetInvulnerable(L4_75, false)
    SetTargetable(L4_75, true)
  else
    L4_75 = MIDDLE_TOWER
    if A2_73 == L4_75 then
      L4_75 = A1_72 + 1
      L4_75 = L3_74[L4_75]
      L4_75.Turret2 = false
      L4_75 = GetTurret
      L4_75 = L4_75(A0_71, A1_72, BACK_TOWER)
      SetInvulnerable(L4_75, false)
      SetTargetable(L4_75, true)
    else
      L4_75 = BACK_TOWER
      if A2_73 == L4_75 then
        L4_75 = A1_72 + 1
        L4_75 = L3_74[L4_75]
        L4_75.Turret1 = false
        L4_75 = GetDampener
        L4_75 = L4_75(A0_71, A1_72)
        SetInvulnerable(L4_75, false)
        SetTargetable(L4_75, true)
      else
        L4_75 = HQ_TOWER2
        if A2_73 == L4_75 then
          L3_74.HQTower2 = false
          L4_75 = L3_74.HQTower1
          if L4_75 == false then
            L4_75 = GetHQ
            L4_75 = L4_75(A0_71)
            SetInvulnerable(L4_75, false)
            SetTargetable(L4_75, true)
          end
        else
          L4_75 = HQ_TOWER1
          if A2_73 == L4_75 then
            L3_74.HQTower1 = false
            L4_75 = L3_74.HQTower2
            if L4_75 == false then
              L4_75 = GetHQ
              L4_75 = L4_75(A0_71)
              SetInvulnerable(L4_75, false)
              SetTargetable(L4_75, true)
            end
          end
        end
      end
    end
  end
end
DeactivateCorrectStructure = L0_0
function L0_0(A0_76, A1_77)
  local L2_78, L3_79, L4_80
  L3_79 = TEAM_ORDER
  if A0_76 == L3_79 then
    L3_79 = OrderBarracksBonuses
    L4_80 = A1_77 + 1
    L2_78 = L3_79[L4_80]
  else
    L3_79 = ChaosBarracksBonuses
    L4_80 = A1_77 + 1
    L2_78 = L3_79[L4_80]
  end
  return L2_78
end
GetLuaBarracks = L0_0
function L0_0(A0_81, A1_82)
  barrack = GetLuaBarracks(A1_82, A0_81)
  return DISABLE_MINION_SPAWN_BASE_TIME + DISABLE_MINION_SPAWN_MAG_TIME * barrack.NumOfSpawnDisables
end
GetDisableMinionSpawnTime = L0_0
function L0_0(A0_83, A1_84)
  cLangBarracks = GetBarracks(A1_84, A0_83)
  luaBarrack = GetLuaBarracks(A1_84, A0_83)
  SetDisableMinionSpawn(cLangBarracks, GetDisableMinionSpawnTime(A0_83, A1_84))
  luaBarrack.NumOfSpawnDisables = luaBarrack.NumOfSpawnDisables + 1
end
DisableBarracksSpawn = L0_0
L0_0 = 0
BonusesCounter = L0_0
function L0_0(A0_85, A1_86)
  local L2_87, L3_88, L4_89, L5_90, L6_91, L7_92, L8_93, L9_94, L10_95
  BonusesCounter = L2_87
  for L5_90 = 1, 3 do
    L6_91, L7_92 = nil, nil
    L8_93 = TEAM_ORDER
    if A0_85 == L8_93 then
      L7_92 = TEAM_CHAOS
      L6_91 = OrderBarracksBonuses
      L8_93 = ChaosBarracksBonuses
      EnemyBarracks = L8_93
    else
      L7_92 = TEAM_ORDER
      L6_91 = ChaosBarracksBonuses
      L8_93 = OrderBarracksBonuses
      EnemyBarracks = L8_93
    end
    L8_93, L9_94 = nil, nil
    L10_95 = L6_91[L5_90]
    L10_95 = L10_95.MinionInfoTable
    for _FORV_14_, _FORV_15_ in pairs(L10_95) do
      _FORV_15_.HPBonus = _FORV_15_.HPBonus + _FORV_15_.HPInhibitor
      _FORV_15_.DamageBonus = _FORV_15_.DamageBonus + _FORV_15_.DamageInhibitor
      _FORV_15_.ExpGiven = _FORV_15_.ExpGiven - _FORV_15_.ExpInhibitor
      _FORV_15_.GoldGiven = _FORV_15_.GoldGiven - _FORV_15_.GoldInhibitor
    end
    if L5_90 == A1_86 + 1 then
      L6_91[L5_90].WillSpawnSuperMinion = 1
      if A0_85 == TEAM_ORDER then
        totalNumberOfChaosBarracks = totalNumberOfChaosBarracks - 1
      else
        totalNumberOfOrderBarracks = totalNumberOfOrderBarracks - 1
      end
    end
  end
end
ApplyBarracksDestructionBonuses = L0_0
L0_0 = 0
ReductionCounter = L0_0
function L0_0(A0_96, A1_97)
  local L2_98, L3_99, L4_100, L5_101, L6_102, L7_103, L8_104, L9_105, L10_106, L11_107, L12_108, L13_109, L14_110, L15_111, L16_112, L17_113, L18_114
  ReductionCounter = L2_98
  for L5_101 = 1, 3 do
    L6_102, L7_103, L8_104 = nil, nil, nil
    L9_105 = TEAM_ORDER
    if A0_96 == L9_105 then
      L8_104 = TEAM_CHAOS
      L6_102 = OrderBarracksBonuses
      L7_103 = ChaosBarracksBonuses
    else
      L8_104 = TEAM_ORDER
      L6_102 = ChaosBarracksBonuses
      L7_103 = OrderBarracksBonuses
    end
    L9_105, L10_106 = nil, nil
    L11_107 = L6_102[L5_101]
    L11_107 = L11_107.MinionInfoTable
    for L15_111, L16_112 in L12_108(L13_109) do
      L17_113 = L16_112.HPBonus
      L18_114 = L16_112.HPInhibitor
      L17_113 = L17_113 - L18_114
      L16_112.HPBonus = L17_113
      L17_113 = L16_112.DamageBonus
      L18_114 = L16_112.DamageInhibitor
      L17_113 = L17_113 - L18_114
      L16_112.DamageBonus = L17_113
      L17_113 = L16_112.ExpGiven
      L18_114 = L16_112.ExpInhibitor
      L17_113 = L17_113 + L18_114
      L16_112.ExpGiven = L17_113
      L17_113 = L16_112.GoldGiven
      L18_114 = L16_112.GoldInhibitor
      L17_113 = L17_113 + L18_114
      L16_112.GoldGiven = L17_113
    end
    if L5_101 == L13_109 then
      if A0_96 == L13_109 then
        totalNumberOfChaosBarracks = L13_109
      else
        totalNumberOfOrderBarracks = L13_109
      end
      L13_109.WillSpawnSuperMinion = 0
    end
    if L12_108 == 3 then
      HQ = L13_109
      L13_109(L14_110, L15_111)
      L13_109(L14_110, L15_111)
      for L16_112 = RIGHT_LANE, LEFT_LANE do
        L17_113 = GetTurret
        L18_114 = L8_104
        L17_113 = L17_113(L18_114, L16_112, HQ_TOWER1)
        L18_114 = GetTurret
        L18_114 = L18_114(L8_104, L16_112, HQ_TOWER2)
        if L17_113 ~= Nil then
          SetInvulnerable(L17_113, true)
          SetTargetable(L17_113, false)
        end
        if L18_114 ~= Nil then
          SetInvulnerable(L18_114, true)
          SetTargetable(L18_114, false)
        end
      end
    end
  end
end
ApplyBarracksRespawnReductions = L0_0
L0_0 = 0
ReactiveCounter = L0_0
function L0_0(A0_115, A1_116)
  local L2_117
  ReactiveCounter = ReactiveCounter + 1
  L2_117 = OppositeTeam(A0_115)
  dampener = GetDampener(A0_115, A1_116)
  SetInvulnerable(dampener, false)
  SetTargetable(dampener, true)
  ApplyBarracksRespawnReductions(L2_117, A1_116)
end
BarrackReactiveEvent = L0_0
function L0_0(A0_118, A1_119)
  local L2_120, L3_121
  if A0_118 == TEAM_ORDER then
    L2_120 = OrderBarracksBonuses[A1_119 + 1]
    L3_121 = ChaosBarracksBonuses[A1_119 + 1]
  elseif A0_118 == TEAM_CHAOS then
    L2_120 = ChaosBarracksBonuses[A1_119 + 1]
    L3_121 = OrderBarracksBonuses[A1_119 + 1]
  end
  if L3_121 then
    L3_121.WillSpawnSuperMinion = 0
  end
end
DisableSuperMinions = L0_0
L0_0 = 0
DisactivatedCounter = L0_0
function L0_0(A0_122)
  local L1_123, L2_124, L3_125
  L1_123 = GetHQType
  L2_124 = A0_122
  L1_123 = L1_123(L2_124)
  HQType = L1_123
  L1_123 = HQType
  L2_124 = ORDER_HQ
  if L1_123 ~= L2_124 then
    L1_123 = HQType
    L2_124 = CHAOS_HQ
  elseif L1_123 == L2_124 then
    L1_123 = HQType
    L2_124 = CHAOS_HQ
    if L1_123 == L2_124 then
      L1_123 = EndOfGameCeremony
      L2_124 = TEAM_ORDER
      L3_125 = A0_122
      L1_123(L2_124, L3_125)
    else
      L1_123 = EndOfGameCeremony
      L2_124 = TEAM_CHAOS
      L3_125 = A0_122
      L1_123(L2_124, L3_125)
    end
    return
  end
  L1_123 = IsDampener
  L2_124 = A0_122
  L1_123 = L1_123(L2_124)
  if L1_123 then
    L1_123 = GetLinkedBarrack
    L2_124 = A0_122
    L1_123 = L1_123(L2_124)
    barrack = L1_123
    L1_123 = GetTeamID
    L2_124 = barrack
    L1_123 = L1_123(L2_124)
    barrackTeam = L1_123
    L1_123 = GetLane
    L2_124 = A0_122
    L1_123 = L1_123(L2_124)
    barrackLane = L1_123
    L1_123 = DisableBarracksSpawn
    L2_124 = barrackLane
    L3_125 = barrackTeam
    L1_123(L2_124, L3_125)
    L1_123 = SetDampenerState
    L2_124 = A0_122
    L3_125 = DampenerRegenerationState
    L1_123(L2_124, L3_125)
    L1_123 = SetInvulnerable
    L2_124 = A0_122
    L3_125 = true
    L1_123(L2_124, L3_125)
    L1_123 = SetTargetable
    L2_124 = A0_122
    L3_125 = false
    L1_123(L2_124, L3_125)
    L1_123 = DisactivatedCounter
    L1_123 = L1_123 + 1
    DisactivatedCounter = L1_123
    L1_123 = GetTurret
    L2_124 = barrackTeam
    L3_125 = 1
    L1_123 = L1_123(L2_124, L3_125, HQ_TOWER1)
    L2_124 = GetTurret
    L3_125 = barrackTeam
    L2_124 = L2_124(L3_125, 1, HQ_TOWER2)
    L3_125 = Nil
    if L1_123 ~= L3_125 then
      L3_125 = SetInvulnerable
      L3_125(L1_123, false)
      L3_125 = SetTargetable
      L3_125(L1_123, true)
    end
    L3_125 = Nil
    if L2_124 ~= L3_125 then
      L3_125 = SetInvulnerable
      L3_125(L2_124, false)
      L3_125 = SetTargetable
      L3_125(L2_124, true)
    end
    L3_125 = Nil
    if L1_123 == L3_125 then
      L3_125 = Nil
      if L2_124 == L3_125 then
        L3_125 = GetHQ
        L3_125 = L3_125(barrackTeam)
        SetInvulnerable(L3_125, false)
        SetTargetable(L3_125, true)
      end
    end
    L3_125 = nil
    if barrackTeam == TEAM_CHAOS then
      L3_125 = TEAM_ORDER
    else
      L3_125 = TEAM_CHAOS
    end
    ApplyBarracksDestructionBonuses(L3_125, barrackLane)
  end
  L1_123 = IsTurretAI
  L2_124 = A0_122
  L1_123 = L1_123(L2_124)
  if L1_123 then
    L1_123 = GetTeamID
    L2_124 = A0_122
    L1_123 = L1_123(L2_124)
    L2_124 = GetObjectLaneId
    L3_125 = A0_122
    L2_124 = L2_124(L3_125)
    L3_125 = GetTurretPosition
    L3_125 = L3_125(A0_122)
    DeactivateCorrectStructure(L1_123, L2_124, L3_125)
    return
  end
  L1_123 = GetDampenerType
  L2_124 = A0_122
  L1_123 = L1_123(L2_124)
  if L1_123 > -1 then
    L2_124 = 0
    L3_125 = TEAM_ORDER
    if L1_123 % TEAM_CHAOS >= RIGHT_LANE and L1_123 % TEAM_CHAOS <= LEFT_LANE then
      L2_124 = ChaosBarracksBonuses[L1_123 % TEAM_CHAOS + 1]
      ChaosBuildingStatus[L1_123 % TEAM_CHAOS + 1].Barracks = false
    else
      L3_125 = TEAM_CHAOS
      L2_124 = OrderBarracksBonuses[L1_123 % TEAM_CHAOS - TEAM_ORDER + 1]
      OrderBuildingStatus[L1_123 % TEAM_CHAOS - TEAM_ORDER + 1].Barracks = false
    end
  else
    L2_124 = Log
    L3_125 = "Could not find Linking barracks!"
    L2_124(L3_125)
  end
  L2_124 = true
  return L2_124
end
HandleDestroyedObject = L0_0
function L0_0()
  local L1_126
  L1_126 = 2
  CANNON_MINION_SPAWN_FREQUENCY = L1_126
end
IncreaseCannonMinionSpawnRate = L0_0
function L0_0(A0_127)
  POST_GAME_EVENTS = {}
end
PostGameSetup = L0_0
function L0_0(A0_128, A1_129)
  local L2_130, L3_131, L4_132, L5_133
  for L5_133, _FORV_6_ in L2_130(L3_131) do
    if A0_128 > _FORV_6_.delay then
      ClientSide_CameraMoveCameraFromCurrentPositionToPoint(_FORV_6_.cameraLocation, _FORV_6_.travelTime)
      if _FORV_6_.soundFile then
        ClientSide_PlaySoundFile(_FORV_6_.soundFile)
      end
      table.remove(POST_GAME_EVENTS, L5_133)
      break
    end
  end
end
PostGameUpdate = L0_0
